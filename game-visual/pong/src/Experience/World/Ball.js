...
update()
{
  ...
  if (!this.ballStopped) {
    ...
    // this.mesh.position.x += this.velocity.x * this.time.delta;
    // this.mesh.position.z += this.velocity.z * this.time.delta;
  }
  //Apply gravity to vertical velocity 
  this.mesh.position.y = -((this.mesh.position.z) * (this.mesh.position.z) / 2100) - 10;
  ...
  if (this.isPastPaddle1()) {
    ...
    setTimeout(() => {this.ballStopped = false;
    }, 1000);
  }
...
processCpuPaddle() {
    ...
    socket.onmessage = function(event) {
        ...
        console.log("JSON recibido del servidor:", currentStatus);
        if (currentStatus.positionpad2) {
            // Extraer la parte relevante de currentStatus y actualizar la posición de paddle2
            self.paddle2.mesh.position.x = currentStatus.positionpad2.x;
            // self.userID = 1;
            // self.gameID = 1;
        }
    };
